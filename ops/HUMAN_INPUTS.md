...
- [ ] **Privacy notice publication (ADR-071.4).** The notice for your instance goes in a separate repo on GitHub Pages (it describes your instance, not the tool). It will name you as controller, a **dedicated contact alias** (please don't use or write your personal e-mail here), purpose, sources, what is and isn't stored (roles and buckets, no handles), retention (report final + 12 months), legal basis, how to opt out or object (the opt-out fingerprint), and US processing. **The draft will be shown to you before anything is published.** Needed from you: approval of the draft, and the contact alias. Until the notice is published, the person-level sources held by ADR-022 (HN and Bluesky mention search) stay off (CB-12, ADR-049.2).

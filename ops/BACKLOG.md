...
- [x] [M21] Cleanup and privacy — accepted 2026-09-26 (verifier round 2) — roles/buckets model + migrate + purge handles and pseudonyms (keep opt-out HMAC only); bot flag on coded records; snapshot purge (report final + 12 months); LLM_BACKEND=api wiring, per-stage models, Batch API, prompt caching; cost estimator vs caps; brief schema v1.2 (money_usd); briefs dir ~/.pigtail/briefs + backup; M12 follow-ups; anomaly checks (ADR-070)
- [~] [M22] Discovery, relevance filter (Haiku, batched), shortlist review UI, `pigtail run --brief` with checkpoints; every stage passes a per-item estimate to BudgetGuard.before_submit (M21 verifier note); selection stage built (outcome sort, winners and matched losers, balance, sensitivity; migration 0022, ADR-077); verifier round-2 fixes done (ADR-078: fresh budget checks, pre-registration gate `pigtail brief preregister` + migration 0023, headline-first matching selection-v2, refusal re-checks, bulk-decision precision label, as_of in the data version), awaiting verifier round 3; follow-ups done 2026-09-26: shortlist carry-forward between brief versions (ADR-079, migration 0024), example brief ranks on attention, `make_guard` fresh ledger reader (ADR-080); verifier round-3 fixes done (ADR-081: per-repo HN launch lookup in the selection fetch, 3 Algolia requests per shortlisted repo, checkpointed; same-day launch precedes a day-precision burst; `anchor-v2` in the pre-registered params, selection-v3; warnings name small populations and missing anchors; anchor type per pair side); verifier round-4 fixes done (ADR-082: title matches only in the `Show/Launch HN: <name>` product slot, no other GitHub link, not before repo creation, 5+ chars and not on the stop-list, rejected candidates counted; selection refused without the Show HN connector, exit 8; `tags=launch_hn`; anchor-v3 / selection-v4 with a code-hash guard test; estimate counts the lookup while the selection is pending), awaiting verifier round 5; open: brief 1 pre-registration amendment 2 (selection-v4), the acceptance selection re-run under anchor-v3 (outcome connectors moved to M23b)
...
## Next
- [ ] [M22-S] Selection follow-ups (ADR-077): outcome connectors now milestone M23b (below; ADR-080), verified business signals still here; founder audience proxy once cleared (outcome-model O14; until then `unknown` for all); issues/downloads/mentions channels for the anomaly checks; `outcome_observation` records per repo and anchor (§1.1) instead of values inside the selection rows; brief schema `min_population_n` (O17); show the stored selection on `/briefs/<id>` (API + UI)
- [ ] [M22-F] M22 follow-ups: widening-step searches (R4.10); metadata fetch for repos added by URL; Run button in the web app; calibrate the planning model from measured runs (M23)
...
- [ ] [M23b] Outcome connectors (WORK_ORDER §4.5 M23b, ADR-080) — M23; before the owner's second report — npm downloads (TM-08), crates.io downloads (TM-09 conditions), `comm.returning_external_contributors` from GitHub PRs (counts only, no handles); optional and off: PyPI via BigQuery (paid, USD 0 cap for other paid services, H6), deps.dev dependents (TM-12); synthetic briefs ranking on adoption and on community get winners and matched losers; verifier sign-off
- [ ] [M22-C] Carry-forward follow-ups (ADR-079): a web-app button; decide whether `status`/`budget.*` changes may join the allow-list
...
- [ ] [M27] (operator guide: remove the subscription-as-default and product-call subscription_share text; update milestone refs, flagged by the M20 verifier) Operator guide (Directive §11 step 8) — M24, M25, M26 — new user runs a first brief in under an hour with the example brief (D6 timed test); 3 consecutive scheduled runs with alerts; "Responsible use" section (ADR-066.8); compliance template (ADR-067); D6 criteria; verifier sign-off
...
- [x] M22-P (done, ADR-078.7: `bulk_id`/`bulk_verdict`, migration 0023; the web page says "Finalize is disabled: N candidates undecided") — precision label: when every decision on a filter verdict comes from one bulk action on that same verdict, label precision "not item-reviewed" instead of "<role>-checked" (R4.7; owner brief v5 showed 100 % after a bulk accept). Also: the web page should say why Finalize is disabled (n undecided).

...
## Next
- [ ] [M22-S] Selection follow-ups (ADR-077): outcome connectors so adoption, community and business dimensions have data (registry downloads npm/PyPI/crates with the primary-ecosystem rule, deps.dev dependents, PR-based returning contributors and external authors, verified business signals); founder audience proxy once cleared (outcome-model O14; until then `unknown` for all); issues/downloads/mentions channels for the anomaly checks; `outcome_observation` records per repo and anchor (§1.1) instead of values inside the selection rows; brief schema `min_population_n` (O17); show the stored selection on `/briefs/<id>` (API + UI)
- [ ] [M22-F] M22 follow-ups: widening-step searches (R4.10); metadata fetch for repos added by URL; carry shortlist decisions over between brief versions; Run button in the web app; calibrate the planning model from measured runs (M23)

...
- [~] [M22] Discovery, relevance filter (Haiku, batched), shortlist review UI, `pigtail run --brief` with checkpoints; every stage passes a per-item estimate to BudgetGuard.before_submit (M21 verifier note); selection stage built (outcome sort, winners and matched losers, balance, sensitivity; migration 0022, ADR-077), awaiting verifier

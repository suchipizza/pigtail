...
## ADR-079 — Carry a final shortlist forward to a new brief version (2026-09-26)
Context: a brief edit that changes only the success definition (primary dimension, threshold, minimums, fallback steps) starts a new version, and a new version started an empty shortlist, so the user had to run discovery and relevance again (paid) and review the same candidates again. Discovery, the relevance rubric and the review read only other brief fields (ADR-055.3, ADR-076.4). Generic for any user. Code: `pigtail.briefs.carry`, `pigtail brief shortlist carry-forward <id> [--from N] [--to M] --as ROLE --reason R [--json]`; migration 0024.
1. **Allow-list, computed generically.** The two versions' full model dumps are compared path by path (`diff.field_changes`); only `success.*`, `panel.*`, `report.*`, `notes` and store metadata (`METADATA_FIELDS`) may differ. Any other difference (`field.*`, `window.*`, `expansion`, `distribution_exemplars`, `status`, `budget.*`, …) is refused with the list of differing paths, and nothing is written. A new brief field is therefore refused until it is added to the allow-list on purpose.
2. **Preconditions:** the source shortlist is final; the target is a later version of the same brief with no candidates, no decisions and no shortlist row. A second call to the same target is refused (not idempotent by design: the first call's rows stand). The copy runs in one transaction under the brief's run lock (refused while a run of the brief is in progress).
3. **What is copied:** every candidate row (verdict, reason, distance, rubric version, relevance provenance, discovery signals, resolution, metadata) with `carried_from_version`; each candidate's **latest** decision as a new immutable `shortlist_decision` row of the target version that keeps the original decision, reason, reviewer role, channel, bulk id and verdict, and `decided_at`, and adds `carried_from_version`, `carried_role`, `carried_reason` and `carried_at` (all four or none, a table check); the named-project resolutions and confirmations (they live on the candidate rows). The target shortlist is then finalized by `Shortlist.finalize` with the carry-forward's role (via `cli`), which writes its `final` mention scope; `brief_shortlist` records `carried_from_version` and `carried_reason`.
4. **Precision** is recomputed from the copied decisions, so it carries the source's label (owner-checked, "not item-reviewed", "not reviewed", …) with "; carried from vN" appended, in the stored record and in `shortlist show` (`precision.carried_from_version`).
5. **Refusal list (CB-13)** is checked again for every candidate: a repo refused since the source was finalized is not copied (nor its decision, nor its scope entry) and is counted in the result (count only); a named project confirmed as that repo goes back to `unresolved` (rule `confirmation_refused`); refused repos are removed from unresolved projects' candidate matches.
6. **Run linkage: a new `brief_runs` row** (chosen over pointing the target shortlist at the source run) with status `carried_forward`, `carried_from` = the source shortlist's run, the source's prompt and model versions, its window end (`checkpoint.run_date`, so a later `--incremental` and the selection use the same window) and discovery, relevance and shortlist marked done with `carried_from_version`/`carried_from_run`. The runner counts `carried_forward` as complete, so `pigtail run --brief <id>` runs only the selection on that row, behind the target version's own pre-registration gate (R8.2; its content hash differs from the source's); after the selection the row ends `succeeded` like any other, and the carry stays visible in `carried_from` and `stages`. Reason: the runner, `--dry-run`, `--incremental` and the provenance of the selection (`brief_selection.brief_run_id`) keep working per version without special cases. README evidence linked to the source version's runs is linked to the new row too (retention anchor, R19.9).
7. **Storage and privacy:** no new table; the new columns hold version numbers, roles, the operator's reason and times; no brief text (a test checks the target's rows add no brief value the source rows didn't already hold, and that the project, named projects and notes appear nowhere). Purge (`REPO_TABLES`), export and inventory already cover the tables touched.
How to reverse: widen or narrow the allow-list through an ADR; carried rows can be told apart by `carried_from_version`.

## ADR-080 — The example brief ranks on attention; outcome connectors are milestone M23b (2026-09-26)
Context: the owner decided (2026-09-26) that her first report ranks on attention, because adoption and community metrics have no connector (ADR-077.2: `unknown`, reason `no_connector`, never imputed). The shipped synthetic example ranked on adoption and got 0 winners.
1. **Example brief** (`docs/examples/brief-example.yaml`): `success.primary: attention`, `primary_threshold: top_quartile`, no minimums (no dimension other than attention has data), fallback steps `[relax_primary_to_top_third]` (a `drop_<dim>_minimum` step needs that minimum). A comment says adoption and community need M23b. Tests that assumed adoption now use attention; a carry-forward test ranks v2 on adoption and checks it gets no winners and says so.
2. **Milestone M23b — Outcome connectors** (WORK_ORDER v2.2 §4.5), after M23 (pilot) and before the owner's second report: npm downloads (TM-08, cleared), crates.io downloads (TM-09, cleared with conditions), `comm.returning_external_contributors` from GitHub pull requests (counts only, no handles); PyPI via BigQuery (TM-07) optional and paid, off under the owner's USD 0 cap for paid services other than the API (ADR-072.4; H6 to enable); deps.dev dependents (TM-12) optional. M24 does not depend on it (the first report ranks on attention). `ops/HUMAN_INPUTS.md` notes that briefs ranking on adoption or community get no winners until M23b ships.
3. **Round-2 left-open item:** `expansion.make_guard` takes `brief_ledger` and the CLI and web app pass a fresh cost-ledger reader, like the runner (ADR-078.6).
How to reverse: the example's success definition by an edit; the milestone order by a WORK_ORDER amendment and ADR.

## ADR-081 — M22 verifier round 3: per-repo launch lookup, same-day anchor rule, anchor rule in the pre-registered hash (2026-09-26; amends ADR-077.2 and ADR-077.3)
Context: verifier M22 round 3 @3c94033. Outcome-model §2.1 defines a declared launch as any Show HN or Launch HN post, whether discovery found it or not, but the anchor used only the Show HN posts recorded in a candidate's discovery sources. On the live acceptance run discovery had found 13 of 19 in-window launches, so launched repos with little traction got no anchor and left both the population and the loser pool: selection on the outcome. `choose_anchor` also compared a launch's exact time with a day-precision burst onset (00:00 Pacific), so a launch on the onset day counted as after the burst. And `Context.params()` held nothing about anchors, so a change to the anchor code passed an existing pre-registration. Code: `pigtail.briefs.outcomes` (`lookup_launches`, `match_launch_post`, `choose_anchor`), `selection` (`SELECTION_VERSION` `selection-v2` → `selection-v3`, `ANCHOR_RULE_VERSION` `anchor-v2`), `connectors.hn` (`search_show_hn(tags=…)`, `launch_lookup_evidence_url`), `estimate`, `privacy.requests`.
1. **Launch lookup (amends ADR-077.2: `att.hn_points` and the anchor no longer rest on discovery's Show HN posts alone).** The selection's fetch step first searches HN Algolia for every shortlisted repo, inside the brief's window, with 3 requests: Show HN (`tags=show_hn`) for `github.com/<owner>/<name>`, Show HN for `<name>`, and all stories (`tags=story`) for `Launch HN <name>`. Algolia's API documentation lists no Launch HN tag (story, comment, poll, pollopt, show_hn, ask_hn, front_page, author_…, story_…; not re-checked live: no network in this session), so the third search keeps only titles that start with "Launch HN". The connector asks only for `title, url, points, created_at_i` and `parse_show_hn_page` never reads the author; the raw page is dropped right after parsing (CB-24, as in discovery). Stored per post, as a candidate signal `hn_launch_lookup`: item id, time, points, kind (`show_hn` | `launch_hn`) and match (`url` | `title`); no author, no title, no text. The evidence record's URL names only the repo and the search (`…/search?launch_lookup_repo=<owner/name>&search=<label>&tags=<tags>`), not the query string or window, and a repo opt-out finds and deletes these rows by that prefix (CB-13c). Requests go through the connector's limiter (5,000 requests/hour). The step is checkpointed per repo (`launch_lookup_done`); a failed request propagates (the run ends `failed`, resumable) and the next run continues with the first repo not done, so no selection is computed on a half-done lookup. With the connector off the step is skipped and the selection says so in its warnings. The estimate counts it (`hn_launch_lookup`: 3 × shortlisted).
2. **Matching rule (conservative).** A hit is a declared launch of the repo when (a) its URL normalises (`normalize_github_repo`: case, `www.`, `.git`, deeper paths, query and fragment ignored) to the repo's `owner/name`: the outcome-model A2 URL rule, case-insensitive; or (b) it links no other GitHub repo and its title names the repo's name as a whole word, case-insensitively: the name is not preceded by a letter, digit, `_`, `-` or `.`, and not followed by a letter, digit, `_`, `-`, or `.` plus a letter or digit (so "KubeForge." at a sentence end and "org-k/KubeForge" match; "Kubeforger", "kubeforge-ui", "kubeforge.io" and "kube_forge" don't). Names shorter than 4 characters match by URL only. A title match that two shortlisted repos share, or whose item another shortlisted repo's post links by URL, is dropped at load. Launches from discovery and the lookup are merged by item id (the lookup's record wins: later points, and it records how it matched), then used both for the anchor and for `att.hn_points`. The anchor records `via` (`discovery`, `lookup:url`, `lookup:title`) so title-matched anchors can be reviewed. Launch type is `show_hn`, `launch_hn` or `burst`.
3. **Day-precision comparison (amends ADR-077.3).** Against a burst onset with `day` precision (all onsets today, outcome-model §2.1), a launch is compared by its endpoint day `D(t)` (§1.2 day mapping, US Pacific): it precedes the burst when `D(t) ≤` the onset day (rules 1 and 5 of §2.2), it lies in `[T_burst − 30 d, T_burst]` when `D(t) ≥` onset day − 30, and a burst follows it within 90 days when `D(t) ≤` onset day `< D(t) + 90`. Hour-precision onsets still compare instants. Example: a post at 02:30 UTC on 1 Aug (19:30 Pacific on 31 Jul) and a burst whose onset day is 31 Jul anchor on the launch; the first star day is 31 Jul, the burst day, as §1.2 intends.
4. **Anchor-rule versioning.** `Context.params()`, hashed into `selection_params_sha256`, now holds `anchor_rule_version` (`anchor-v2`: launch lookup plus day-precision comparison), a one-line `anchor_rule` and the `launch_lookup` rule text; `SELECTION_VERSION` is `selection-v3`. Any change to `choose_anchor` or the lookup must bump `ANCHOR_RULE_VERSION`, which changes the hash, so the gate (ADR-078.5) refuses a pre-registration recorded before it. Consequence: brief 1 v6's pre-registration (`2026-09-26-brief-1.md`, `selection-v2`) no longer passes; brief 1 needs a new pre-registration file before its outcome sort.
5. **Warnings say why there are few winners:** `no anchor: <n> of <m> shortlisted` (exemplars left out) and, per dimension the final definition uses, `<dimension> population <n> < 20 (minimum): no percentiles` (per group for downloads). The summary also counts `anchors` by type and source and `pairs_by_anchor`.
6. **H1-type contrasts (MC-01) are reported within launch-anchored pairs only.** Losers need a declared launch to have an anchor (a repo without traction has no burst), while winners can be anchored on a burst, and `launch_type` isn't an exact-match key; so over all pairs, "a first-party HN launch is more common among winners" is tilted by the anchor rule itself (every loser has one by construction). Chosen: restrict, not label. A "confounded" label would still show a number whose direction the design fixes in advance, and the headline table would carry it; restricting keeps only pairs where both sides were selected the same way. Within launch-anchored pairs MC-01's *presence* is equal on both sides by construction, so the report states "presence not testable under this anchor rule" and contrasts only what differs among launches (MC-01's preconditions, the post's timing and reception, and the related MC-02/MC-03/MC-11/MC-13 fields); burst-anchored winners with a launch are counted descriptively, never as a contrast. Data: every pair side stores `anchor_type`, each loser row stores `anchor_types` ({winner, loser}) and `launch_anchored`, and the summary has `pairs_by_anchor` and `headline_pairs_launch_anchored`, so the report can filter. The brief-1 pre-registration that replaces the `selection-v2` one should state this for H1.
How to reverse: items 1–3 only before a brief's outcome sort, by a new ADR with a new `ANCHOR_RULE_VERSION` and selection version; item 6 by an ADR before the report that uses it.

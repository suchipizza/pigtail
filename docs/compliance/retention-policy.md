...
| `person_level_30d` | Per-repo GitHub event actor rows (pseudonymised `WatchEvent` and `ForkEvent` actors, repo, time) and the raw per-repo events snapshots (TM-33, DPIA D12, ADR-036) | **Raw events: dropped at parse** (hash and URL kept, `raw_dropped`; CB-23). **Actor rows: ≤ 30 days from `fetched_at`** | Delete the actor rows; only aggregates without pseudonyms remain (bot-removed counts, lockstep flags, `coverage_ratio`). Periods above 30 days rejected at startup; table registered in `PERSON_TABLES`. | **P (CB-22)**, with M1-T24. The connector stays off (`PIGTAIL_ENABLE_GITHUB_EVENTS=0` plus the ADR-022 flag) until then. |

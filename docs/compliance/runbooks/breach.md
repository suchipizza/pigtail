...
| **UI audit log** (`ui_audit_log`) | `login_failure`, `login_rate_limited`, unexpected `login_success` or `snapshot_view` | I (logged). Alerts `login_failures` (failed or rate-limited logins over a threshold) and `snapshot_integrity` (any integrity failure, critical) are raised from it, counts only (**I CB-30**, ADR-046.5). Logins at unusual times are not alerted: review the log. |
...
- **CB-30** (done, ADR-046.5): alerts from the UI audit log for repeated failed logins and any `snapshot_integrity_failure`; logins at unusual times are still not alerted.
- **CB-25** (done, ADR-045): an accidental `PSEUDONYM_KEY` change is now detected and refused ([key-rotation.md](key-rotation.md) §0, §6).
- **CB-17b** (partly done, ADR-046.6: doctor backup checks, optional backup jobs): deletions made after the last backup are lost if the live database is lost too; ship `deletion_log` and opt-outs off the host continuously (open).
- **CB-31** (done, ADR-046.5): the host alert files (`ALERTS.md`, `alerts.jsonl`) rotate at 1 MB or 30 days and rotated files are deleted within `LOG_RETENTION_DAYS` (≤ 365).
...
- 2026-09-25 — ADR-046 (CB-17b/30/31): §2 audit-log alerts implemented (CB-30); tooling gaps updated (CB-30, CB-31 done; CB-17b partial).

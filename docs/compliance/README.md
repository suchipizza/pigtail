...
| [dpia.md](dpia.md) | Light DPIA: whether a DPIA is required (yes), the data inventory D1–D14, risks R1–R15 rated by likelihood and severity, measures mapped to implemented or planned controls, residual risk, and **backlog CB-01…CB-35** | GDPR Art. 35–36; WP248 rev.01; FADP Art. 22–23 |
...
| [runbooks/key-rotation.md](runbooks/key-rotation.md) | `PSEUDONYM_KEY` management: where it lives, separate backup, what a rotation breaks, rotation with `pigtail privacy rekey` (manual procedure only as fallback), scheduled rotation every 24 months, restore guard for pre-rotation backups, compromise response (CB-09 v0.3; CB-25, CB-26, CB-28, CB-29 and CB-35 done, CB-27 not needed) | GDPR Art. 4(5), 32; FADP Art. 8; EDPB Guidelines 9/2022 ¶77 |
...
- **Legitimate interest:** supported for mention capture, LLM coding, scoring, the library and planner, and aggregate publication, on condition that the safeguards exist. **Account-level spread graphs are on hold** (LQ-8). The GH Archive velocity scan, which the LIA also supported, was **removed 2026-09-26 (ADR-051); its data was deleted by migration 0014 except count-only `deletion_log` rows** ([lia.md](lia.md) A1). A brief-restricted GH Archive use (M13) needs a new assessment first.
- **Controls that must exist before the capture layer runs on the production host** (ADR-022 as amended by ADR-043, `ops/DECISIONS.md`; status table in [dpia.md](dpia.md) §7):
  ...
  - CB-09 key management (**documented**: [runbooks/key-rotation.md](runbooks/key-rotation.md) v0.3; key-change detection CB-25 ✓; rotation by re-derivation `pigtail privacy rekey` CB-26 ✓; dual-key matching CB-27 not needed (ADR-046.2); LLM cache clear CB-28 ✓; restore refuses backups taken before the last `rekey` or `reset` CB-35 ✓; scheduled rotation allowed again, every 24 months, ADR-046.3)
  ...
  - CB-17 encrypted backups (**✓ done**, ADR-044.2–3: `pigtail backup create|restore|prune`, restore re-applies deletions; follow-ups CB-17b **partial**, ADR-046.6: `pigtail doctor` checks `BACKUP_RECIPIENT` and backup age, optional scheduler jobs ship disabled; still open: continuous off-host shipping of `deletion_log` and opt-outs, a backup container; the LLM cache is not in backups)
  ...
  - CB-25 key-change detection (**✓ done**, ADR-045; added by ADR-043: key fingerprint recorded on first use, every key-using path, backup restore and scheduler startup refuse on a mismatch, `pigtail doctor` check, `pigtail privacy key-fingerprint --reset --confirm-rotation` only after the runbook's §4)
  - CB-29 secret hygiene (**✓ done**, ADR-045; required before production by ADR-043: the `ui` service gets an explicit environment without `PSEUDONYM_KEY`, secrets masked in `repr(Settings)`)
...
- **Code status:** the capture-layer controls (pseudonymisation at ingest in `connectors/base.py`, `ConnectorGapError`, the bot drop (`velocity.py` removed with the velocity scan in M11, ADR-051), the `retention_class` and `deletion_state` fields, CB-04 raw purge) are merged to `main` and labelled "I". The privacy operations of ADR-030 are merged at `83843ca`: the retention purge (CB-01), LLM cache expiry and evidence links (CB-05), redactor extensions (CB-06, partly), access and erasure requests (CB-08), the opt-out list (CB-13), log scrubbing (CB-18, partly) and the `pigtail doctor` encryption check (CB-03, partly). Code: `src/pigtail/privacy/`, `src/pigtail/logsafe.py`, `src/pigtail/pseudonymize.py`, `src/pigtail/llm/store.py`, `migrations/0003_privacy_operations.sql`; operator commands in `docs/guides/operator.md` "Privacy operations". Since then (M3-T11 status sync): the redacting log filter runs on every CLI command and scheduled job (CB-18, ADR-040.6); raw GitHub search pages are dropped at parse (CB-24, ADR-040.5); name opt-outs are keyed (CB-13b, ADR-042.1) and the repo purge covers every repo-keyed table (CB-13c, ADR-044.1); encrypted backups with deletion-preserving restore exist (CB-17, ADR-044.2–3); the private UI has an operator login and an audit log (CB-19, ADR-034; role-based access open). ADR-045: key-change detection (CB-25), secret hygiene for the UI and `Settings` (CB-29), `GHARCHIVE_RAW_RETENTION_DAYS` capped at 30 (CB-32), UI audit rows and expired sessions purged by the daily retention job (CB-33), and unparseable snapshots no longer abort access, erasure or opt-out purges (CB-34). ADR-046: atomic key rotation `pigtail privacy rekey` (CB-26; CB-27 not needed), `pigtail llm cache clear` (CB-28), alerts from the UI audit log (CB-30), alert-file rotation (CB-31), doctor backup checks and optional backup jobs (CB-17b, partial), and restore refuses backups taken before the last `rekey` or bare fingerprint reset (CB-35). GH Archive capture ran locally in development only and was removed on 2026-09-26 (ADR-051); there is no production capture (ADR-022).
- **GitHub collection after the M11 re-scope (ADR-047.6, ADR-049.1, ADR-051):** the watch list, its GraphQL counts, the all-GitHub search sweeps and screens, and global detection (detection v1) were **removed 2026-09-26 (ADR-051); their data was deleted by migration 0014 except count-only `deletion_log` rows** (reason `purpose_limitation`; [retention-policy.md](retention-policy.md) §2.1). What remains: (a) the project-level `github` connector fetches **per-repo star history** for the repos of open cases once `GITHUB_TOKEN` is set (daily job `gh_star_history`); a brief-scoped `search_repos` is built (raw pages dropped right after parsing, CB-24) but nothing calls it until M13. (b) The person-level `github_events` connector (per-repo star and fork events for repos with a live case, ADR-051.6) is built with **CB-22** (`person_level_30d`, named after its 30-day ceiling: `GITHUB_EVENTS_RETENTION_DAYS` is 16 days by default (ceiling 30; ADR-038), daily purge) and **CB-23** (only `WatchEvent`/`ForkEvent`, raw pages dropped at parse with hash and tombstone kept, no stargazer-list path; tested), but stays **off by default** (`PIGTAIL_ENABLE_GITHUB_EVENTS` plus the ADR-022 flag; ADR-036) until CB-12 exists (CB-03 and CB-06 partly done). There is no GitHub deletion-sync source; CB-02 for this source is met by the 16-day expiry plus the raw drop at parse (ADR-038), and LQ-29 items 4–5 stay open for the lawyer. (c) The **HN connectors**: mentions stay gated (ADR-022, ADR-031.2); the project-level **HN rank poller** takes one front-page snapshot per scheduled run and polls continuously only in launch mode (ADR-049.1, ADR-051.4). Cases that detection v1 opened before the removal stay in the cache with `bot_filter` `unavailable`.
- **LLM:**
...
- The rest of the backup follow-ups (CB-17b: off-host shipping of `deletion_log` and opt-outs, a backup container); see [dpia.md](dpia.md) §9.
...
- 2026-09-25 — ADR-045 (CB-25/29/32/33/34): key conclusions mark CB-25 and CB-29 done and CB-09 at runbook v0.2; code status adds CB-25, CB-29, CB-32, CB-33 and CB-34; backlog count CB-01…CB-34; "Not in this pack yet" narrowed to CB-26…CB-28.
- 2026-09-25 — ADR-046 (CB-17b/26/27/28/30/31/35): key conclusions mark CB-26, CB-28 and CB-35 done, CB-27 not needed, CB-17b partial and CB-09 at runbook v0.3 (rotation with `privacy rekey`, scheduled rotation allowed again); code status adds ADR-046; file index: runbook description and backlog count CB-01…CB-35; "Not in this pack yet" narrowed to the CB-17b remainder.
- 2026-09-26 — sync to the M11 removal (ADR-047.6, ADR-049.1, ADR-051; migration 0014): file index (RoPA activities), key conclusions (legitimate interest no longer lists the velocity scan) and code status updated; the GitHub detection v1 paragraph is replaced by what remains after the re-scope (per-repo star history, gated per-repo events, gated HN mentions, HN rank poller per ADR-049.1), with the removed processing marked removed 2026-09-26 (ADR-051) and its data deleted by migration 0014 except count-only `deletion_log` rows.

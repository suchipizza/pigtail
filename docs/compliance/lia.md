...
- **Scale:** A1 covers every public GitHub event, so millions of people. A1b covers people who star repos with an open case (the default schedule polls only those): unknown until the connector runs. With the optional pre-threshold set enabled it grows a lot: on one measured day about 1,078 repos gained ≥ 30 stars (detection-replan §7.4), so polling that whole set would see tens of thousands of star events a day (arithmetic lower bound ≈ 32,000). A2–A4 cover people who mention roughly 300 Tier 2 repos (G2).

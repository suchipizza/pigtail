...
| **Stargazer identity from per-repo events (A1b)** | R1.1 counts stars "after bot filtering", and R3.3 removes fake-star campaigns. Both need to know which stars in a window came from bot or lockstep accounts. The star-history endpoint gives counts only (ADR-032). | (a) Star-history only, no filter: R1.1's filter step and R3.3 would be unmet, and the filtered series `unknown` everywhere. (b) Drop bot logins by name in memory without storing anything: **adopted** for the name rule (as S3). The lockstep flag compares actors across hours and repos, so it needs stable pseudonyms for a while. (c) GH Archive only: ~2 % of stars in 2026 (ADR-028), too little to filter. (d) Keep pseudonymised star rows for ≤ 30 days (the lockstep window, Δt = 30 d), for tracked repos only, then aggregates: **adopted** (TM-33 conditions). | Necessary in pseudonymised, ≤ 30-day, open-case-only form. **Built** (M1-T24; CB-22 and CB-23 implemented and tested, [dpia.md](dpia.md) §6 R15 and §9); the connector is off by default. **Finding (proportionality):** option (d) was justified by the 30-day lockstep window, but detection v1 does not apply the lockstep rule to per-repo events (ADR-037.2). Today the pseudonyms only de-duplicate stars within a case window (48 h, cases polled for 14 days) and feed daily distinct counts. A shorter `GITHUB_EVENTS_RETENTION_DAYS` (for example 16 days: the 14-day case window plus the 48-h detection window) would serve the current use; 30 days is justified only if a lockstep or cross-hour layer is added. Recommendation to the orchestrator: lower the default or record why 30 days stays. |
...
- **A1b (per-repo stargazer events) is not enabled.** S16 now exists (M1-T24; CB-22 and CB-23 implemented and tested, ADR-037). The per-repo Events API is a person-level GitHub source other than GH Archive, so the ADR-022 person-level hold applies to it (every pre-condition above, including CB-02), plus CB-22 (retention class `person_level_30d`) and CB-23: **ADR-036 (adopted 2026-09-25)**. The connector is gated by `PIGTAIL_ENABLE_GITHUB_EVENTS` (default off) plus the ADR-022 flag `PIGTAIL_ADR022_PERSON_SOURCES_OK`. What still holds it is the rest of the ADR-022 list: CB-02 (deletion sync: the HN source exists, the Bluesky source does not, and there is no GitHub deletion-sync source; for this source the ≤ 30-day expiry of actor rows and the drop of raw pages at parse are the mitigation, and whether CB-02 applies to it at all is for the orchestrator, since ADR-036 lists it) and CB-12 (published notice), with CB-03 and CB-06 partly done. LQ-29 does not block it under the TM-33 defaults, but its answer can.
...
| A1b stargazer events (per-repo Events, open cases) | **Hold.** S16 exists (M1-T24; CB-22 and CB-23 implemented, ADR-037); the hold remains until the ADR-022 person-level pre-conditions are met (open: CB-02, CB-12; partly done: CB-03, CB-06) (ADR-036, adopted 2026-09-25; gate `PIGTAIL_ENABLE_GITHUB_EVENTS`, default off, plus the ADR-022 flag; retention class `person_level_30d`); proceed then under the TM-33 conditions, pending LQ-29. Star counts from the star-history endpoint (no personal data) **proceed**. |

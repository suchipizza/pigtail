...
| A1b | **Stargazer events for tracked repos (bot-filter confirmation).** The per-repo Events API is polled every 15–60 min, only for repos with an open case, tracked repos (R17.4) or repos above the pre-threshold, so that the R1.1 bot filter and the lockstep flag can run on who starred or forked in the window (ADR-032.2). Star counts themselves come from the star-history endpoint, which holds no personal data (ADR-032.3). **Not built yet (M1-T24).** | R1.1, R3.3, R17.4, TM-33, LQ-29 | `WatchEvent.actor` and `ForkEvent.actor` (login and id) with repo and time, pseudonymised at ingest and kept as person-level rows for **at most 30 days** (retention class `person_level_30d`, ADR-036), then only aggregates (bot-removed counts, lockstep flags, `coverage_ratio`). The raw response also holds the repo's other recent public events (pushes, issues, PRs, with actors and payload text); only `WatchEvent`s and `ForkEvent`s are parsed (fork actors are kept because the bot and lockstep features need them to detect fork farms, and the PRD §8.1 attention metrics count forks). Never used to build, store or export a stargazer list. |
...
| S16 | **Stargazer-event limits (A1b; TM-33 conditions, LQ-29 default):** per-repo events only for repos with an open case, tracked repos or repos above the pre-threshold; polling no faster than `X-Poll-Interval` (15–60 min, ETag); one operator token; `actor` pseudonymised at ingest; only `WatchEvent`s and `ForkEvent`s parsed (fork actors are kept because the bot and lockstep features need them to detect fork farms, and the PRD §8.1 attention metrics count forks) and the raw response dropped after parse; person-level star and fork rows kept ≤ 30 days (`person_level_30d`), then only aggregates; identities used only for bot and lockstep flags; no stargazer list is ever built, stored, shown or exported, and no cross-repo stargazer graph beyond the tracked set; each case records `bot_filter.basis` and `coverage_ratio`; the connector stays off (`PIGTAIL_ENABLE_GITHUB_EVENTS=0`, default, plus the ADR-022 flag) until ADR-036's conditions are met. | P: M1-T24 (connector), CB-22 (`person_level_30d` retention class and purge), CB-23 (parse minimisation and no-list guard; ADR-036). S1 (pseudonymiser) is I and would be reused. |
...
| A1b stargazer events (per-repo Events, tracked repos) | **Hold** until S16 exists (M1-T24, CB-22, CB-23) and the ADR-022 person-level pre-conditions are met (ADR-036, adopted 2026-09-25; gate `PIGTAIL_ENABLE_GITHUB_EVENTS`, default off, plus the ADR-022 flag; retention class `person_level_30d`); proceed then under the TM-33 conditions, pending LQ-29. Star counts from the star-history endpoint (no personal data) **proceed**. |

...
|---|---|---|---|
| A1 | **Velocity scan — removed 2026-09-26 (ADR-051); data deleted by migration 0014 except count-only `deletion_log` rows.** Hourly GH Archive dumps were ingested to detect star or fork bursts across GitHub. The scan, its backfill and its jobs are removed (ADR-047.6); migration 0014 dropped `repo_hourly_activity` and `gharchive_hours` after logging their row counts (reason `purpose_limitation`). Raw dumps already stored expire within 30 days (CB-04). The GH Archive connector is kept for brief-restricted discovery signals (ADR-047.1, M13) but fetches nothing today; that use needs its own assessment here before it runs. | R1.1 (retired), TM-01 | Formerly: GitHub login of every actor (pseudonymised when parsed), event type, repo and timestamp; raw hourly dump kept as a snapshot. Now: only raw dumps awaiting their 30-day purge. |
| A1b | **Stargazer events for tracked repos (bot-filter confirmation).** The per-repo Events API is polled every 15–60 min, only for repos with a live case opened in the last 14 days (any live case since ADR-051.6; the watch-list pre-threshold targets were removed with the watch list, ADR-051.1), so that the R1.1 bot filter can run on who starred or forked in the window (ADR-032.2). Only the login-based bot rules are applied; the lockstep rule is not, because per-repo events cannot show an account's activity elsewhere (ADR-037.2). Star counts themselves come from the star-history endpoint, which holds no personal data (ADR-032.3). **Built in M1-T24 (`src/pigtail/connectors/github.py` `GitHubRepoEventsConnector`, `src/pigtail/capture/repo_events.py`); off by default** (`PIGTAIL_ENABLE_GITHUB_EVENTS` plus the ADR-022 flag). | R1.1, R3.3, R17.4, TM-33, LQ-29 | `WatchEvent.actor` and `ForkEvent.actor` (login and id) with repo and time, pseudonymised at ingest and kept as person-level rows for **16 days by default (ceiling 30; ADR-038)** (retention class `person_level_30d`, ADR-036; the class is named after its 30-day ceiling, while the operative default is 16 days), then only aggregates without pseudonyms (daily distinct star and fork counts and bot counts per repo in `repo_event_daily_agg`; per case `stars_seen`, `stars_bot`, `coverage_ratio`, `confirmed`). The raw response also holds the repo's other recent public events (pushes, issues, PRs, with actors and payload text); only `WatchEvent`s and `ForkEvent`s are parsed (fork actors are kept because fork farms are part of the bot features, and the PRD §8.1 attention metrics count forks), and the raw bytes are dropped right after parsing. Never used to build, store or export a stargazer list. |
...
|---|---|---|---|
| **Actor identity for GitHub events (A1; removed 2026-09-26, ADR-051, kept here as the record of the assessment)** | Star velocity must count distinct actors per repo-hour. The lockstep fake-star filter (R3.3) needs per-actor features within a window. Returning contributors (§8.1) need a stable actor id across months. | (a) Aggregate counts only: this defeats bot and fake-star filtering. (b) A stable keyed pseudonym instead of the login: **adopted** (`src/pigtail/pseudonymize.py`, on `main`; applied at ingest by `src/pigtail/connectors/base.py` `records()`, **I**). Per-actor features are held in memory for one window only and never persisted (`src/pigtail/capture/velocity.py`, **I**). Bot logins are dropped without hashing (`src/pigtail/connectors/gharchive.py`, **I**). | Necessary in pseudonymised form. **Raw logins in the stored hourly snapshot** go beyond what the analysis needs; they are kept only for replay (PRD §5.5) and integrity, and purged after ≤ 30 days (CB-04, partly implemented: `capture/retention.py`). See DPIA R4. |
| **Stargazer identity from per-repo events (A1b)** | R1.1 counts stars "after bot filtering", and R3.3 removes fake-star campaigns. Both need to know which stars in a window came from bot or lockstep accounts. The star-history endpoint gives counts only (ADR-032). | (a) Star-history only, no filter: R1.1's filter step and R3.3 would be unmet, and the filtered series `unknown` everywhere. (b) Drop bot logins by name in memory without storing anything: **adopted** for the name rule (as S3). An in-memory drop cannot tell whether a star seen in one poll is the same account as a star seen in a later poll of the same case. (c) GH Archive only: ~2 % of stars in 2026 (ADR-028), too little to filter. (d) Keep pseudonymised star and fork rows only as long as a case window lasts, for tracked repos only, then aggregates: **adopted** (TM-33 conditions). The lockstep rule is not applied to per-repo events, because they cannot show an account's activity elsewhere (ADR-037.2), so the 30-day lockstep window (Δt = 30 d) is not a purpose for this source. | Necessary in pseudonymised, short-lived, open-case-only form. **Purpose of the retained pseudonyms:** de-duplicating stars and forks within a case window (the 48-h detection window plus the 14-day case cooldown) and feeding daily distinct counts, which needs about 16 days. Hence `GITHUB_EVENTS_RETENTION_DAYS` defaults to **16 days (ceiling 30; ADR-038)**; going above 16 needs a documented purpose (for example a lockstep or cross-hour layer added by an ADR). **Built** (M1-T24; CB-22 and CB-23 implemented and tested, [dpia.md](dpia.md) §6 R15 and §9); the connector is off by default. |
...
- **Scale:** A1 covered every public GitHub event, so millions of people; it was removed on 2026-09-26 (ADR-051). A1b covers people who star repos with an open case: unknown until the connector runs. The optional pre-threshold set, which would have grown it to tens of thousands of star events a day (detection-replan §7.4), was removed with the watch list (ADR-051.1). A2–A4 cover people who mention roughly 300 Tier 2 repos (G2).
...
| S3 | Bot logins are dropped. Per-actor features are never persisted. | I: login rules in `src/pigtail/capture/botfilter.py`, applied before hashing in `src/pigtail/connectors/gharchive.py` and `src/pigtail/connectors/github.py`. The per-actor lockstep features were computed only by the removed velocity scan (`velocity.py`, ADR-051); no remaining code computes them |
| S4 | No public release of person-level data, raw snapshots or identifying spread graphs. The app is private by default. | I (policy): PRD §4, R13.3. The CI private-data scan is I (`scripts/private_data_scan.py`). App authentication is I (CB-19, ADR-034): operator login, server-side sessions and an audit log of logins, logouts and snapshot views without IP addresses (`src/pigtail/api/auth.py`, `migrations/0006_ui_auth_audit.sql`). Role-based access is P (one operator account only). |
...
| S6 | Deletion sync where platforms require it, and as a courtesy for HN `deleted`. | Partly I. The field `evidence.deletion_state` is I (`migrations/0001_capture_v0.sql`). CB-02 per source: **HN** implemented (`pigtail privacy deletion-sync --source hn`); **GitHub per-repo events** met by short retention (16 days by default, ceiling 30) plus the raw drop at parse, no sync source (ADR-038); **GH Archive** raw dumps purged after ≤ 30 days (CB-04) and replay drops opted-out persons at ingest (CB-13); **Bluesky** pending (no connector yet). |
...
| S13 | Explicit refusals are honoured (FADP Art. 30(2)(b); e.g. a Bluesky user-intents opt-out once adopted, TM-06). | I: CB-13 (opt-out list of pseudonyms and repo ids, checked at ingest in `src/pigtail/connectors/base.py`, purge of existing data); CB-13b (repo opt-outs by name stored as keyed hashes, ADR-042.1); CB-13c (the repo purge reaches every repo-keyed table in `REPO_TABLES`, with a schema test, ADR-044.1). Consuming a Bluesky user-intents signal is P (no Bluesky connector yet). |
...
| S16 | **Stargazer-event limits (A1b; TM-33 conditions, LQ-29 default):** per-repo events only for repos with a live case opened in the last 14 days (ADR-051.6; pre-threshold targets removed, ADR-051.1); polling no faster than `X-Poll-Interval` (15–60 min, ETag); one operator token; `actor` pseudonymised at ingest, bot logins dropped before hashing; only `WatchEvent`s and `ForkEvent`s parsed and the raw response dropped right after parse (hash kept, tombstone); person-level star and fork rows kept 16 days by default (ceiling 30; ADR-038) (`person_level_30d`, `GITHUB_EVENTS_RETENTION_DAYS`), then only aggregates; identities used only for the login-based bot flag and for de-duplicating stars and forks within a case window (no lockstep on per-repo events, ADR-037.2); no stargazer list is ever built, stored, shown or exported, and no cross-repo stargazer graph; each case records `bot_filter.basis`, `confirmed` and `coverage_ratio`; the connector stays off (`PIGTAIL_ENABLE_GITHUB_EVENTS=0`, default, plus the ADR-022 flag) until ADR-036's conditions are met. | **I** (M1-T24, ADR-037): connector `src/pigtail/connectors/github.py` (`GitHubRepoEventsConnector`: `KEPT_EVENT_TYPES`, `_parse`, `_pre_pseudonymize`, `enabled_by_default = False`, `person_level_hold = True`); poller `src/pigtail/capture/repo_events.py` (`targets()`, `EventsConfig`, `drop_after_parse` in `_ingest`, aggregate-only reads); CB-22 in `migrations/0007_github_detection.sql`, `src/pigtail/privacy/retention.py`, `src/pigtail/privacy/deletion.py` (`PERSON_TABLES`), `src/pigtail/config.py`; tests in `tests/unit/test_github_privacy_m1t24.py` and `tests/integration/test_github_per_repo_m1t24.py`. The gate is in force: the ADR-022 pre-condition CB-12 is still open and CB-03 and CB-06 are partly done; CB-02 is met for this source by ADR-038. |
...
For **A1b (stargazer events)** the balance is **narrow**, because GitHub's 2026 restriction lowers what users expect (§4.2). It holds only if S16 is implemented in full, and it is subject to LQ-29. The short retention supports it: pseudonyms are kept only as long as de-duplication within a case window needs (16 days by default, ceiling 30; ADR-038), not for a cross-account lockstep analysis, which is not applied to this source (ADR-037.2). If the lawyer answers LQ-29 against it, A1b stops, the filtered star series stay `unknown` (ADR-032.3), and nothing is imputed.
...
- A1 was **removed on 2026-09-26 (ADR-051); its data was deleted by migration 0014 except count-only `deletion_log` rows**. The conditions below still apply to any capture that runs on the production host (the per-repo star history, the HN rank poller, and the gated person-level connectors). Production capture on the host waits for the ADR-022 production controls, as amended by ADR-043 (CB-01, CB-03, CB-04, CB-09, CB-12, CB-16, CB-17, CB-18, CB-25; ADR-043 also requires CB-29). Status: CB-01 ✓; CB-03 partial (enabling bucket SSE on the host and encrypting the Postgres volume are operator duties); CB-04 partial; CB-09 documented (runbook v0.3; CB-25 and CB-26 ✓, CB-27 not needed, ADR-046); CB-12 open (owner details); CB-16 documented; CB-17 ✓ (ADR-044); CB-18 ✓ (ADR-040.6; host log rotation is an operator duty); CB-25 open (in progress); CB-29 open (in progress). The authoritative list and statuses are in [dpia.md](dpia.md) §7. The M1 safeguards (S1 at ingest, S3, S12) are merged to `main`. H2 "does not block collection under the documented safeguards" (WORK_ORDER §5).
- **No person-level source beyond GH Archive (Bluesky, HN, V2EX, Discord) is enabled** until every pre-condition in **ADR-022** exists: CB-01 (retention purge, S5), CB-02 (deletion sync, S6), CB-03 (encryption at rest), CB-06 (identifier redaction before LLM calls, S2 extensions), CB-08 (data-subject request tooling, S7), CB-12 (published notice, S8) and CB-13 (honouring refusals, S13). ADR-022 is the single authoritative list. Status (ADR-030, ADR-038): CB-01, CB-08 and CB-13 implemented; CB-03 and CB-06 partly implemented; CB-02: HN done / GitHub events met via ADR-038 / Bluesky pending; CB-12 open.
- **A1b (per-repo stargazer events) is not enabled.** S16 now exists (M1-T24; CB-22 and CB-23 implemented and tested, ADR-037). The per-repo Events API is a person-level GitHub source other than GH Archive, so the ADR-022 person-level hold applies to it (every pre-condition above, including CB-02), plus CB-22 (retention class `person_level_30d`) and CB-23: **ADR-036 (adopted 2026-09-25)**. The connector is gated by `PIGTAIL_ENABLE_GITHUB_EVENTS` (default off) plus the ADR-022 flag `PIGTAIL_ADR022_PERSON_SOURCES_OK`. What still holds it is the rest of the ADR-022 list: CB-12 (published notice), with CB-03 and CB-06 partly done. CB-02 is met for this source by ADR-038: raw pages are dropped at parse and actor rows expire after 16 days by default (ceiling 30), so upstream un-stars and deletions are reflected within that period; there is no GitHub deletion-sync source, and one is added only if GitHub or H2 requires it (LQ-29 item 4). LQ-29 does not block it under the TM-33 defaults, but its answer can.
...
2. S1, S2, S3, S4 and S12 are active. All five are on `main`.
...
4a. For stargazer events from per-repo polling (A1b): condition 3 (ADR-036, adopted 2026-09-25), plus S16 in full (including CB-22, retention class `person_level_30d`, and CB-23), the gate `PIGTAIL_ENABLE_GITHUB_EVENTS` (default off) set together with the ADR-022 flag, and no contrary answer to LQ-29.
...
| Activity | Outcome |
|---|---|
| A1 velocity scan | **Removed 2026-09-26 (ADR-051); data deleted by migration 0014 except count-only `deletion_log` rows.** Raw dumps already stored expire within 30 days (CB-04). A future brief-restricted GH Archive use (M13) needs a new assessment first. |
| A1b stargazer events (per-repo Events, open cases) | **Hold.** S16 exists (M1-T24; CB-22 and CB-23 implemented, ADR-037); the hold remains until the ADR-022 person-level pre-conditions are met (open: CB-12; partly done: CB-03, CB-06; CB-02 met for this source by ADR-038) (ADR-036, adopted 2026-09-25; gate `PIGTAIL_ENABLE_GITHUB_EVENTS`, default off, plus the ADR-022 flag; retention class `person_level_30d`); proceed then under the TM-33 conditions, pending LQ-29. Star counts from the star-history endpoint (no personal data) **proceed**. |
...
- 2026-09-25 — fixes after verifier (ADR-036 alignment): A1b, S16 and §4.4 name `WatchEvent` and `ForkEvent` actors (forks needed for fork-farm bot/lockstep features and the PRD §8.1 attention metrics); §4.5 condition list, §6 condition 4a and §7 A1b row now cite ADR-036 (adopted 2026-09-25) instead of a proposed ADR-022 amendment, and name the gate `PIGTAIL_ENABLE_GITHUB_EVENTS` (default off, plus the ADR-022 flag) and the retention class `person_level_30d`; `bot_filter_basis` renamed `bot_filter.basis` (case schema).
- 2026-09-25 — CB-22/23 implemented (M1-T24, ADR-037): A1b marked built and off by default, with its actual scope (live velocity cases ≤ 14 days; pre-threshold repos only with `--prethreshold`) and aggregates; the lockstep rule is recorded as not applied to per-repo events (ADR-037.2); §3 adds a proportionality finding on the 30-day period now that lockstep is not applied (recommendation to the orchestrator); §4.3 scale updated; S16 marked I with code and test citations; §4.5 and §7 name the remaining ADR-022 holds (CB-02 with no GitHub deletion-sync source, CB-12).
- 2026-09-25 — ADR-038 wording (16-day default; CB-02 per source)
- 2026-09-25 — status sync (M3-T11): S4 app authentication implemented (CB-19, ADR-034; role-based access open); S13 adds CB-13b and CB-13c; §4.5 production-precondition status per ADR-022 as amended by ADR-043 (CB-17 and CB-18 done; CB-25 and CB-29 listed).

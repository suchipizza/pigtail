...
- pigtail's own log records go through the redacting filter on every command and scheduled job (CB-18, ADR-040.6). Rotate container and system logs at 12 months yourself (journald or logrotate). pigtail rotates the host alert files and deletes rotated ones within `LOG_RETENTION_DAYS` (**I CB-31**, ADR-046.5).
...
- **`PSEUDONYM_KEY`**: generate, store and back up exactly as in [runbooks/key-rotation.md](runbooks/key-rotation.md) §2. Keep it apart from data backups, and record its `kfp1_` fingerprint (`running_key_fingerprint` from `pigtail privacy key-fingerprint`) in your deployment notes. pigtail refuses to run under a different key (CB-25): if `pigtail doctor` reports `pseudonym_key_fingerprint` FAIL and you did not rotate on purpose, restore the original key; never reset. Rotate with `pigtail privacy rekey` following the runbook's §4.1: every 24 months, after a compromise, and when someone with access to the key leaves (ADR-046.3). Keep the original opt-out requests so every opt-out can be mapped at the next rotation (runbook §4.3; LQ-30). After a rotation, take a fresh backup: older ones can no longer be restored (CB-26, CB-35). Under Compose the `ui` service does not receive the key (CB-29); keep it that way if you edit `docker-compose.yml`.
...
- **Private UI:** keep it on loopback; use an SSH tunnel or a TLS reverse proxy; use a strong password (`pigtail ui hash-password`). There is one operator account and no role-based access (CB-19 remainder, **P**): everyone with the password sees everything, so give it only to staff who need full access. Alerts are raised for repeated failed logins and snapshot-integrity failures (**I CB-30**, ADR-046.5); still review `ui_audit_log` weekly for anything else (for example logins at unusual times).
...
- 2026-09-25 — ADR-046 (CB-26/30/31/35): key rotation with `pigtail privacy rekey` on a 24-month schedule (runbook v0.3 §4.1, §4.3) and fresh backup afterwards (CB-35); §4 alert-file rotation implemented (CB-31); private UI audit-log alerts implemented (CB-30).

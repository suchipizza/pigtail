...
| CB-24 | Drop raw GitHub search pages at parse: only the owner type is used, but the pages embed owner objects and are kept as `person_level_24m` today (D14(a); ADR-038). Keep hash and URL, write a `deletion_log` tombstone, as for HN items. | Before GitHub search sweeps run in production | engineer — planned (backlog CB-24) |
| CB-25 | Detect a `PSEUDONYM_KEY` change: store a key fingerprint (an HMAC of a fixed label, never the key) in the database at first use; `pigtail doctor` FAILs and capture refuses to start on a mismatch. Today an accidental key change silently stops refusal-list matching ([runbooks/key-rotation.md](runbooks/key-rotation.md) §3). | Production host (with CB-09) | engineer — planned |
...
| CB-29 | Secret hygiene: the `ui` service does not receive `PSEUDONYM_KEY` (per-service env files in `docker-compose.yml`; today it loads the whole `.env`); `Settings.pseudonym_key` declared with `repr=False` like `s3_secret_key` (`src/pigtail/config.py`) | Production host | engineer — planned |

...
| D12 | **Stargazer events for tracked repos** (ADR-032.2; **planned, M1-T24; not built**) | `WatchEvent` actor login and id, `ForkEvent` actor login and id, repo, time (fork actors are kept because the bot and lockstep features need them to detect fork farms, and the PRD §8.1 attention metrics count forks). The raw per-repo response also holds that repo's other recent public events (pushes, issues, PRs, with actors and payload text). | Per-repo Events API (TM-33 under TM-02; actor use pending LQ-29), only for repos with an open case, tracked repos or repos above the pre-threshold, polled every 15–60 min | Raw response: snapshot store, bytes dropped after parse (P, CB-23). Parsed: `WatchEvent`s and `ForkEvent`s only, actor as a pseudonym, in a registered person-level table (ADR-030.5). Aggregates (bot-removed counts, lockstep flags, `coverage_ratio`, `bot_filter.basis`) on the case. | Raw → pseudonymised → aggregate | **`person_level_30d`, ≤ 30 days** for raw bytes and pseudonymised star and fork rows (TM-33, ADR-036; P, CB-22); aggregates `derived_aggregate` | Operator |
...
| | Minimise at parse: keep only `WatchEvent` and `ForkEvent` actor pseudonym, repo and time (forks for fork-farm detection and the PRD §8.1 attention metrics); drop the raw response bytes right after parse (hash and URL kept, `raw_dropped`), as ADR-031.1 does for HN items. | P CB-23 |
| | Retention: the ≤ 30-day class `person_level_30d` for D12 raw bytes and pseudonymised star and fork rows, enforced by `pigtail retention purge` and rejected at startup if set longer; the table registered in `PERSON_TABLES` so erasure reaches it (ADR-030.5). After 30 days only aggregates remain. | P CB-22 |
...
| | Record per case: `bot_filter.basis` and `coverage_ratio`, so the filter's data use is auditable. | P (M1-T24) |
| | Gate: the ADR-022 person-level hold applies (flag `PIGTAIL_ADR022_PERSON_SOURCES_OK`, `person_level_hold = True`), plus CB-22 and CB-23: ADR-036 (adopted 2026-09-25). The connector also needs `PIGTAIL_ENABLE_GITHUB_EVENTS` (default off). | Decided (ADR-036); controls P |
...
| CB-23 | **Required by ADR-036 (ADR-032, TM-33).** Parse minimisation and no-list guard for per-repo events: parse only `WatchEvent` and `ForkEvent` actor pseudonym, repo and time (fork actors are needed for fork-farm bot/lockstep features and the PRD §8.1 attention metrics); drop raw bytes after parse; no API, UI view, export or report that lists a repo's stargazers; no cross-repo stargazer graph beyond the tracked set. Tested. | Per-repo event polling (D12, A1b) | engineer (with M1-T24) |

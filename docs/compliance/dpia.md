...
| D14 | **Watch-list screen inputs** (M1-T24) | (a) GitHub `search/repositories` result pages: each item embeds the repo's `owner` object (login, id, avatar and profile URLs). Only repo id, node id, `owner/name`, counts, dates and the owner **type** are parsed (`parse_search_page`). (b) HN "Show HN" items: the raw item JSON holds the poster's `by` handle; only the GitHub URL is kept. | GitHub Search API (TM-02); HN Firebase via the project-level rank poller (TM-04, ADR-031) | (a) Raw search pages: bytes dropped right after parsing (`drop_after_parse` in `SearchSweeper._page`, `src/pigtail/capture/github_screens.py`; hash and URL kept, evidence `raw_dropped`, tombstone in `deletion_log`; CB-24); a page that fails to parse is dropped at once (CB-23b). (b) HN item bytes dropped right after parsing (`drop_after_parse`, same file) | (a) Raw, person-level (owner objects), dropped at parse / parsed: project-level. (b) Raw dropped at once | (a) Classed **`person_level_24m`** at fetch because results embed owner objects (ADR-037.8; `search_repositories()` in `src/pigtail/connectors/github.py`), but the raw bytes are not retained: dropped right after parsing (**I, CB-24**; ADR-038, ADR-040.5; tested: `test_cb24_search_pages_raw_dropped_after_parse_owner_type_only`, `tests/integration/test_github_raw_drop_cb24_cb23b.py`). (b) raw not retained | Operator |
...
**Minimisation note (D14a).** Only the owner type is used from search pages, so their raw bytes are dropped right after parsing, as for HN items (CB-24, implemented; ADR-040.5). No pool of owner objects is kept; only the hash, URL and a `deletion_log` tombstone remain.
...
| | Key management, backup apart from data, rotation and compromise procedure; the key is never placed in DB dumps ([runbooks/key-rotation.md](runbooks/key-rotation.md)). Rotation tooling (key fingerprint check, re-pseudonymisation, dual-key matching) is not built, so scheduled rotation is suspended until it is. | I (documentation) CB-09 / P CB-25, CB-26, CB-27, CB-28, CB-29 |
...
| | Drop raw GitHub search pages right after parsing (only the owner type is used); unparseable person-level pages dropped at once (`SearchSweeper._page`, `src/pigtail/capture/github_screens.py`; `tests/integration/test_github_raw_drop_cb24_cb23b.py`). | I CB-24, CB-23b |
...
| | Access control and an audit log on the private UI: operator login (argon2id), server-side sessions, login rate limiting, audit log of logins, logouts and snapshot views without IP addresses (ADR-034; `src/pigtail/api/auth.py`, `migrations/0006_ui_auth_audit.sql`). Role-based access is not built (one operator account). Audit-log alerting (CB-30) and purging audit rows in the retention job (CB-33) are open. | I CB-19 (login, audit log) / P CB-19 (role-based access), CB-30, CB-33 |
| | Breach runbook: GDPR Art. 33 ("not later than 72 hours" where feasible); FADP Art. 24 ("as quickly as possible") ([runbooks/breach.md](runbooks/breach.md)). Audit-log alerting is not built. | I (documentation) CB-16 / O (fill in roles) / P CB-30 |
...
| | Container and system log rotation (12 months) on the host. Rotation of the host alert files is CB-31. | O (rotation, CB-18) / P CB-31 |
...
| CB-09 key-management runbook | documented ([runbooks/key-rotation.md](runbooks/key-rotation.md)); rotation tooling CB-25…CB-27 open, so the key is rotated only after a compromise |
...
| CB-17 encrypted backups | ✓ done (ADR-044.2–3); follow-ups open (see §6 R13) |
| CB-18 log hygiene | ✓ done (filter on every command and job, ADR-040.6); host log rotation is an operator duty |
| CB-25 key-change detection (ADR-043) | open (in progress) |
| CB-29 secret hygiene (ADR-043) | open (in progress) |
...
| CB-17 | Backup policy: encrypted, 35-day rotation, deletion tombstones re-applied on restore | Production host | engineer — **Done 2026-09-25** (ADR-044.2–3): `pigtail backup create`, `restore`, `prune` (`src/pigtail/privacy/backup.py`; `tests/integration/test_backup_cb17.py`). Follow-ups open: continuous off-host shipping of `deletion_log` and opt-outs; a backup container or scheduler job; the LLM cache is not in backups (retention-policy §2) |
...
| CB-24 | Drop raw GitHub search pages at parse: only the owner type is used, but the pages embed owner objects and are kept as `person_level_24m` today (D14(a); ADR-038). Keep hash and URL, write a `deletion_log` tombstone, as for HN items. | Before GitHub search sweeps run in production | engineer — **Done 2026-09-25** (ADR-040.5): `drop_after_parse` in `SearchSweeper._page` (`src/pigtail/capture/github_screens.py`); tested in `tests/integration/test_github_raw_drop_cb24_cb23b.py` |
| CB-25 | Detect a `PSEUDONYM_KEY` change: store a key fingerprint (an HMAC of a fixed label, never the key) in the database at first use; `pigtail doctor` FAILs and capture refuses to start on a mismatch. Today an accidental key change silently stops refusal-list matching ([runbooks/key-rotation.md](runbooks/key-rotation.md) §3). | Production host (ADR-022 precondition, added by ADR-043) | engineer — open (in progress) |
| CB-26 | Re-pseudonymisation migration (`pigtail privacy rekey-pseudonyms --old-key-env …`, dry run first): map refusal-list pseudonyms and `rk_` name keys through retained raw snapshots and local data, rewrite or delete pseudonymous rows, clear the LLM cache, report unmappable entries, record the run | Any scheduled key rotation | engineer — planned |
| CB-27 | Dual-key matching during a rotation window: ingest, access and erasure also compute old-key pseudonyms while an old key is configured, so objections keep working and old rows are found | Any key rotation after data exists | engineer — planned |
| CB-28 | LLM cache clear command (all cache rows and evidence links, keeping the usage ledger), for key rotation and incidents | Key rotation once LLM coding runs (M5) | engineer — planned |
| CB-29 | Secret hygiene: the `ui` service does not receive `PSEUDONYM_KEY` (per-service env files in `docker-compose.yml`; today it loads the whole `.env`); `Settings.pseudonym_key` declared with `repr=False` like `s3_secret_key` (`src/pigtail/config.py`) | Production host (ADR-043) | engineer — open (in progress) |
| CB-30 | Alert from the UI audit log: repeated `login_failure`/`login_rate_limited`, any `snapshot_integrity_failure` (breach detection, [runbooks/breach.md](runbooks/breach.md) §2) | Production host with the UI exposed through a tunnel or proxy | engineer — planned |
| CB-31 | Rotate the host alert files (`ALERTS.md`, `alerts.jsonl`; append-only today in `src/pigtail/scheduler/alerts.py`) at 12 months, like other logs (CB-18) | Production host | engineer — planned |
| CB-32 | Cap `GHARCHIVE_RAW_RETENTION_DAYS` at 30 in `src/pigtail/config.py` (today any integer is accepted, unlike the other retention settings) | Production capture on the host | engineer — planned |
| CB-33 | Purge `ui_audit_log` rows past `LOG_RETENTION_DAYS` in `pigtail retention purge` (today only at each login, `create_session()` in `src/pigtail/api/auth.py`, so rows outlive the limit when nobody logs in) | Production host | engineer — planned |

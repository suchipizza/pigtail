...
- **Check 1, spike without matching activity (`spike_no_activity`).**
  - Endpoint day `d` is a spike day when `n(d) ≥ 50` net stars (`min_spike_stars`) and `(n(d) − mean) / σ ≥ 3` (`spike_sigma`), with `mean` over the 30 known days before `d` (`baseline_days`; at least 14 known, `min_baseline_days`, else `d` cannot spike) and `σ = max(sample SD, √mean, 1)` (the √μ floor of ADR-027 item 3; `min_sigma` = 1). Consecutive spike days form one spike.

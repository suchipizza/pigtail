...
**Codebook:** `docs/methodology/codebook.md` v0.3.1 (CB). **Burst code and parameters:** `pigtail.analysis.bursts` (`src/pigtail/analysis/bursts.py`) with `schemas/analysis-params/v1.0.0.json` (mirrored by `pigtail.analysis.params`; ADR-051.3). The category taxonomy formerly in §3.1 of this spec now lives in CB §9.
...
- **Method:** StarScout (He et al., ICSE 2026), the current reference method; no peer-reviewed successor was found (LR §2.1–2.2). Parameters pinned as `starscout-v0` (unchanged from v1): low-activity signature; lockstep (CopyCatch) n = 50, m = 10, Δt = 30 d, ρ = 0.5; campaign post-processing: a month with > 50 fake stars and > 50 % fake share, and fake stars > 10 % of all-time stars (LR §2.1, §2.3). The parameters moved from the retired thresholds JSON to `schemas/analysis-params/v1.0.0.json` (block `fake_star_filter`, mirrored by `pigtail.analysis.params`) in M11 (ADR-051.3); no code applies them yet (M13).

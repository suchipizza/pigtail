...
**Requirements:** PRD §5.3, §5.6, F3 (R3.1–R3.5), §8.1, §8.2, §9.1; ADR-009, ADR-010, ADR-032 (supersedes ADR-012; star source and R1.1 basis, M3-T6).
...
2. **Stars in GH Archive are unusable for scoring, and the stargazers API is closed.** GH Archive has under-captured stars since 2025-05 (ADR-009); on 2026-09-24 it held about 2 % of stars (ADR-028, detection-replan §7.2). GitHub restricted the stargazer lists (REST and GraphQL) to admins and collaborators on 2026-06-30, so ADR-012's source is gone (detection-replan §0). Under **ADR-032** the scoring star series is the daily net count from GitHub's star-history endpoint (§1.2). It has no identities, so the bot- and fake-star-filtered series exist only where identity-level event data exists. **Consequence:** under v0.1.0, which classes on `starscout_filtered` (§4), `A30`, `V90` and `P30` are `unknown` for most retrospective windows, so most retrospective cases are `unclassified:missing_data` until an ADR decides the class series (§5.3, open question O11).
...
- **Defaults:** classes use `starscout_filtered` (v0.1.0, ADR-020). Every class-level finding is re-run on `raw`; findings that flip are flagged `sensitive_to_fake_star_filter` (LR §2.3 step 5). Under ADR-032 `starscout_filtered` is `unknown` for most windows, which leaves most cases without `A30`; see §5.3 and O11.
...
| `V90` | `att.star_velocity_7d@90` (filtered, stars/day over `[T+83d, T+90d)`, i.e. the last 7 endpoint days of the 90-day window, §1.2) |
...
- **Coverage gate (per case, per window; 0.90, unchanged).** Any series built from identity-level events feeds `A30`, `V90` or `P30` only if its `coverage_ratio ≥ 0.90` for the window, measured against star-history day counts for the same endpoint days (§1.2). It applies to (a) an event-archive count standing in for `raw` where star-history cannot serve the window, and (b) the event data behind `bot_filtered` and `starscout_filtered` (class eligibility, §1.2). Below the gate or unmeasured, those inputs are `unknown`.
- **Consequence under v0.1.0.** Classes use `starscout_filtered` (§4). Under ADR-032 it is class-eligible only for windows covered by per-repo event polling (cases tracked from before T), so `A30` is `unknown` and the case `unclassified:missing_data` for almost every retrospective case and for live cases first seen after T. The raw-series classes are still computed and stored as the sensitivity run (§4). Which series classes should use is open question O11 and needs an ADR before the pilot; this spec does not change the class series on its own.
...
- **O11 — Class series under ADR-032.** v0.1.0 classes on `starscout_filtered`, which is `unknown` for most windows now (§4, §5.3). Options: (a) keep it, and accept that most retrospective cases are `unclassified:missing_data` (the pilot as pre-registered could not classify its cases); (b) class on `raw` star-history for every case, with `bot_filtered`/`starscout_filtered` as a sensitivity run where class-eligible; (c) per case, filtered where eligible and raw otherwise (rejected here: one cell would mix two series). (b) and (c) change a class input, so under §5.4 they need a new thresholds version, an ADR and dated amendments to the pre-registrations that name the stargazers API or the filtered series. Proposed to the orchestrator as an ADR; not decided in this spec.
...
12. Star-history effects (ADR-032): the share of cases whose filtered star series is class-eligible, by cohort and entry path; how much `fetch_lag_days` (survivor loss) differs between live and backfilled cases in the same cell; the share of anchors with `day` onset precision; and whether the day mapping (§1.2) moves any case across a class threshold when shifted by one day.
